...
func main() {
	...
	log.Printf("kiff-decide listening on %s (domain: instant-payout-enablement)", *addr)
...
func (s *server) decide(w http.ResponseWriter, r *http.Request) {
	...
	case err == nil:
		writeJSON(w, http.StatusOK, decideResponse{ProposalID: propID, Outcome: "allowed",
			Message: "payout allowed; escrow is cleared and ready to disburse"})
	case errors.Is(err, action.ErrStateNotAllowed):
		writeJSON(w, http.StatusOK, decideResponse{
			...
			Message: fmt.Sprintf("escrow is %q — payout allowed only when CLEARED", currentState),
		})
...
func (s *server) ingest(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil || req.EscrowID == "" || req.Type == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "escrow_id and type are required"})
		return
	}
	actorID := req.ActorID
	if actorID == "" {
	...
	ev := escrowEvent(req.EscrowID, req.Type, actorID, req.Payload)
...
func (s *server) seed(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil || req.EscrowID == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "escrow_id is required"})
		return
	}
	ev := escrowEvent(req.EscrowID, EventEscrowCreated, "system", req.Payload)
	if err := s.rt.IngestEvent(r.Context(), ev); err != nil {
		writeJSON(w, http.StatusBadGateway, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(w, http.StatusCreated, map[string]string{"status": "seeded", "escrow_id": req.EscrowID, "state": StateCreated})
}
