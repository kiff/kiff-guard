# Recipe Manifest: instant-payout-enablement-guard
...
│   ├── domain.go                    # Escrow domain: CREATED→CLEARED→DISBURSED; DISBURSE_PAYOUT from CLEARED
│   │                                # policy-owned roles: AssignRole(payout-agent, payout)
│   ├── go.mod                       # Depends on github.com/kiff/kiff v0.6.0
│   └── go.sum
│
├── app/
│   └── server.py                    # Service 2: payout-app, system of record (stdlib only)
│                                    # /escrow, /clear, /disburse, /ledger, /reset, /healthz
│                                    # non-idempotent /disburse (pays out on every call)
│                                    # port via APP_PORT (default 8082)
│
├── agent/
│   └── payout_agent.py              # Service 3: Agno (v2) agent + KIFF guard
│                                    # build_guard(), create_payout_agent(), run_agent(), make_model()
│                                    # tool: disburse_payout(escrow_id, amount_cents, seller_id)
│
└── driver/
...
PAYOUT_APP_URL=http://localhost:8082
...
3. See the same enablement verdict: the agent disburses the legitimate payout
   the moment escrow CLEARED (allowed), the escrow advances to DISBURSED, and
   every repeat is declined (one payout per escrow)
