// Command kiff-decide — payment-recovery-enablement-guard gate server.
//
// Routes:
//   POST /v1/proposals/decide
//   POST /v1/events/raw       advance invoice state (PAYMENT_FAILED, PAYMENT_RECOVERED)
//   POST /seed                seed an invoice into CREATED
...
func main() {
	...
	log.Printf("kiff-decide listening on %s (domain: payment-recovery-enablement)", *addr)
...
func (s *server) decide(w http.ResponseWriter, r *http.Request) {
	...
	case err == nil:
		writeJSON(w, http.StatusOK, decideResponse{ProposalID: propID, Outcome: "allowed",
			Message: "retry allowed; invoice is past due and recoverable"})
	case errors.Is(err, action.ErrStateNotAllowed):
		writeJSON(w, http.StatusOK, decideResponse{
			...
			Message: fmt.Sprintf("invoice is %q — retry allowed only while PAST_DUE", currentState),
		})
...
func (s *server) seed(w http.ResponseWriter, r *http.Request) {
	...
	ev := invoiceEvent(req.InvoiceID, EventInvoiceCreated, "system", req.Payload)
	if err := s.rt.IngestEvent(r.Context(), ev); err != nil {
		writeJSON(w, http.StatusBadGateway, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(w, http.StatusCreated, map[string]string{"status": "seeded", "invoice_id": req.InvoiceID, "state": StateCreated})
}
