// Command kiff-decide — prompt-injection-refund-guard gate server.
//
// Routes:
//
//	POST /v1/proposals/decide
//	POST /v1/events/raw       advance order state (PAYMENT_CAPTURED, REFUND_ISSUED)
//	POST /seed                seed an order into CREATED
...
func main() {
	...
	log.Printf("kiff-decide listening on %s (domain: prompt-injection-refund)", *addr)
...
func (s *server) decide(w http.ResponseWriter, r *http.Request) {
	...
	case err == nil:
		writeJSON(w, http.StatusOK, decideResponse{ProposalID: propID, Outcome: "allowed",
			Message: fmt.Sprintf("%s allowed; order is %s", req.ActionName, currentState)})
	case errors.Is(err, action.ErrStateNotAllowed):
		writeJSON(w, http.StatusOK, decideResponse{
			...
			Message: fmt.Sprintf("order is %q — %s is not allowed in that state", currentState, req.ActionName),
		})
...
func (s *server) seed(w http.ResponseWriter, r *http.Request) {
	...
	ev := orderEvent(req.OrderID, EventOrderCreated, "system", req.Payload)
	if err := s.rt.IngestEvent(r.Context(), ev); err != nil {
		writeJSON(w, http.StatusBadGateway, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(w, http.StatusCreated, map[string]string{"status": "seeded", "order_id": req.OrderID, "state": StateCreated})
}
