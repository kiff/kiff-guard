// Command kiff-decide — deal-close-enablement-guard gate server.
//
// Routes:
//   POST /v1/proposals/decide
//   POST /v1/events/raw       advance deal state (DEAL_QUALIFIED, DISCOUNT_APPLIED)
//   POST /seed                seed a deal into CREATED
...
func main() {
	...
	log.Printf("kiff-decide listening on %s (domain: deal-close-enablement)", *addr)
...
func (s *server) decide(w http.ResponseWriter, r *http.Request) {
	...
	case err == nil:
		writeJSON(w, http.StatusOK, decideResponse{ProposalID: propID, Outcome: "allowed",
			Message: "discount allowed; deal is open to close"})
	case errors.Is(err, action.ErrStateNotAllowed):
		writeJSON(w, http.StatusOK, decideResponse{
			...
			Message: fmt.Sprintf("deal is %q — discount allowed only while OPEN", currentState),
		})
...
func (s *server) ingest(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil || req.DealID == "" || req.Type == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "deal_id and type are required"})
		return
	}
	actorID := req.ActorID
	if actorID == "" {
	...
	ev := dealEvent(req.DealID, req.Type, actorID, req.Payload)
...
func (s *server) seed(w http.ResponseWriter, r *http.Request) {
	var req ingestRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil || req.DealID == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "deal_id is required"})
		return
	}
	ev := dealEvent(req.DealID, EventDealCreated, "system", req.Payload)
	if err := s.rt.IngestEvent(r.Context(), ev); err != nil {
		writeJSON(w, http.StatusBadGateway, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(w, http.StatusCreated, map[string]string{"status": "seeded", "deal_id": req.DealID, "state": StateCreated})
}
