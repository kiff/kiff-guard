// Command kiff-decide — refund-enablement-guard gate server.
//
// Routes:
...
func main() {
	...
	log.Printf("kiff-decide listening on %s (domain: refund-enablement)", *addr)
...
func (s *server) decide(w http.ResponseWriter, r *http.Request) {
	...
	case err == nil:
		writeJSON(w, http.StatusOK, decideResponse{ProposalID: propID, Outcome: "allowed",
			Message: "refund cleared; order can be refunded"})
	case errors.Is(err, action.ErrStateNotAllowed):
		writeJSON(w, http.StatusOK, decideResponse{
			...
			Message: fmt.Sprintf("order is %q — refund allowed only from PAID", currentState),
		})
